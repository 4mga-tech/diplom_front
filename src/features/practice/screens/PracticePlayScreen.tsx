      ...
      <ScrollView contentContainerStyle={styles.contentContainer}>
            ...
            <View key={task.id} style={styles.questionCard}>
              ...
              <View style={styles.optionsWrap}>
                {task.options.map((option) => {
                  ...
                  return (
                    <Pressable key={option.id} disabled={answered} style={[styles.optionButton, isSelected && styles.optionSelected, answered && isSelected && correct && styles.optionCorrect, answered && isSelected && !correct && styles.optionWrong]} onPress={() => onSelectOption(task, option.id)}>
                      <Text style={styles.optionText}>{option.text}</Text>
                    </Pressable>
                  );
                })}
              </View>
              {answered ? <Text style={[styles.feedbackText, correct ? styles.correctText : styles.wrongText]}>{correct ? "Correct" : "Wrong"}</Text> : null}
            </View>
          );
        })}
        ...
        {playState === "completed" ? (
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultText}>Correct: {attemptResult?.correctAnswers ?? correctCount}/{attemptResult?.totalQuestions ?? totalCount}</Text>
            <Text style={styles.resultText}>XP earned: {attemptResult?.xpEarned ?? 0}</Text>
            <Text style={styles.resultText}>Daily XP: {attemptResult?.dailyXpEarned ?? 0}/{attemptResult?.dailyXpLimit ?? practice.maxDailyXp ?? 0}</Text>
            <Text style={styles.resultText}>XP capped: {attemptResult?.xpCapped ? "Yes" : "No"}</Text>
            <Pressable style={styles.primaryButton} onPress={() => void loadPractice()}><Text style={styles.primaryButtonText}>Retry</Text></Pressable>
          </View>
        ) : (
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: "#F8FAFC" }, contentContainer: { padding: 12, gap: 10, paddingBottom: 20 }, centerContainer: { flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 24, gap: 8 }, centerTitle: { fontSize: 16, fontWeight: "700", color: "#111827" }, centerText: { fontSize: 14, color: "#4B5563", textAlign: "center" },
  headerCard: { backgroundColor: "#FFFFFF", borderColor: "#E5E7EB", borderWidth: 1, borderRadius: 10, padding: 10 }, title: { fontSize: 16, fontWeight: "800", color: "#111827" }, metaText: { marginTop: 4, fontSize: 12, color: "#374151" },
  questionCard: { backgroundColor: "#FFFFFF", borderColor: "#E5E7EB", borderWidth: 1, borderRadius: 10, padding: 10, gap: 8 }, questionNumber: { fontSize: 11, fontWeight: "700", color: "#6366F1" }, prompt: { fontSize: 14, fontWeight: "600", color: "#111827" },
  optionsWrap: { gap: 6 }, optionButton: { borderWidth: 1, borderColor: "#D1D5DB", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, backgroundColor: "#FFFFFF" }, optionSelected: { borderColor: "#6366F1", backgroundColor: "#EEF2FF" }, optionCorrect: { borderColor: "#16A34A", backgroundColor: "#DCFCE7" }, optionWrong: { borderColor: "#DC2626", backgroundColor: "#FEE2E2" }, optionText: { fontSize: 13, color: "#111827" },
  feedbackText: { fontSize: 12, fontWeight: "700" }, correctText: { color: "#15803D" }, wrongText: { color: "#B91C1C" },
  primaryButton: { marginTop: 4, borderRadius: 10, paddingHorizontal: 16, paddingVertical: 10, backgroundColor: "#4F46E5", alignItems: "center" }, primaryButtonText: { color: "#FFFFFF", fontWeight: "700" }, disabledButton: { opacity: 0.45 },
  resultCard: { backgroundColor: "#FFFFFF", borderColor: "#E5E7EB", borderWidth: 1, borderRadius: 10, padding: 12, gap: 6 }, resultTitle: { fontSize: 15, fontWeight: "800", color: "#111827" }, resultText: { fontSize: 13, color: "#1F2937" },
});
